            ...
            <div class="card">
                ...
                <div class="w-100 p-4">
                    ...
                    <div class="form-group row">
                        <input type="hidden" id="id_donatur" value="<?= $donatur->id_donatur ?>">
                        <div class="col-lg-3">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-4">
                            ...
                            <input type="text" value="<?= $donatur->nama_lengkap ?>" class="form-control" id="nama_lengkap">
                        </div>
                        <div class="col-lg-3">
                            <label for="">No. HP</label>
                            <input type="number" value="<?= $donatur->no_hp ?>" class="form-control" id="no_hp">
                        </div>
                        <div class="col-lg-3">
                            <label for="">No. HP 2</label>
                            <input type="number" class="form-control" id="no_hp2" value="<?= $donatur->no_hp2 ?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-lg-3">
                            <label for="">Email</label>
                            <input type="email" value="<?= $donatur->email_donatur ?>"  class="form-control" id="email_donatur">
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-lg-8">
                        ...
                        <div class="col-lg-4">
                            ...
                            <input type="number"  value="<?= $donatur->kode_pos ?>" class="form-control" id="kode_pos">
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-lg-12">
                            <label for="">Address</label>
                            <input type="text"  value="<?= $donatur->address ?>" class="form-control" placeholder="Address" id="address">
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-lg-4">
                            <label for="">NIK / No. KTP / No. Perusahaan</label>
                            <input type="text"  value="<?= $donatur->no_identitas ?>" class="form-control" placeholder="NIK / No. KTP / No. Perusahaan" id="no_identitas">
                        </div>
                        <div class="col-lg-4">
                            <label for="">NPWP</label>
                            <input type="text"  value="<?= $donatur->npwp ?>" class="form-control" placeholder="NPWP" id="npwp">
                        </div>
                        <div class="col-lg-4">
                            <label for="">NPWZ</label>
                            <input type="text" value="<?= $donatur->npwz ?>" class="form-control" placeholder="NPWZ" id="npwz">
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-lg-4">
                        ...
                        <div class="col-lg-4">
                            ...
                            <input type="text" value="<?= $donatur->kode_rekening ?>" class="form-control" placeholder="Kode Rekening" id="kode_rekening">
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-lg-4">
                    ...
                    <div class="form-group row">
                        <div class="col-lg-4">
                            ...
                            <input type="date" value="<?= $donatur->tgl_lahir ?>" class="form-control" id="tgl_lahir">
                        </div>
                        <div class="col-lg-4">
                        ...
                        <div class="col-lg-4">
                            ...
                            <input type="text" value="<?= $donatur->pekerjaan ?>" class="form-control" placeholder="Pekerjaan" id="pekerjaan">
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-lg-4">
...
<script>
    ...
    $(document).ready(function () {
        var tipe_donatur = "<?= $donatur->tipe_donatur ?>";
        var kategori_prospek = "<?= $donatur->kategori_prospek ?>"; 
        var kategori_donatur = "<?= $donatur->kategori_donatur ?>"; 
        var notifikasi = `<?= $donatur->notifikasi ?>`; 
        notifikasi = JSON.parse(notifikasi);
        var sapaan = `<?= $donatur->sapaan ?>`; 
        var status_hidup = `<?= $donatur->status_hidup ?>`; 
        var status_donatur = `<?= $donatur->status_donatur ?>`; 
        var terima_hard_copy = `<?= $donatur->terima_hard_copy ?>`; 
        var jenis_kelamin = `<?= $donatur->jenis_kelamin ?>`;
        var pendidikan_terakhir = `<?= $donatur->pendidikan_terakhir ?>`;
        var id_kota = `<?= $donatur->id_kota ?>`;
        var provinsi = `<?= $donatur->provinsi ?>`;
        var kota = `<?= $donatur->kota ?>`;
        var kecamatan = `<?= $donatur->kecamatan ?>`;

        $("#tipe_donatur").val(tipe_donatur).trigger("change"); 
        $("#kategori_prospek").val(kategori_prospek).trigger("change");  
        $("#kategori_donatur").val(kategori_donatur).trigger("change");  
        for (var key in notifikasi) {
            if (notifikasi[key] == 'true') {
                $(`[name="notifikasi"][value="${key}"]`).attr("checked", true);
            }
        } 
        $("#sapaan").val(sapaan).trigger("change"); 
        $("#status_hidup").val(status_hidup).trigger("change"); 
        $("#status_donatur").val(status_donatur).trigger("change"); 
        $("#terima_hard_copy").val(terima_hard_copy).trigger("change"); 
        $("#jenis_kelamin").val(jenis_kelamin).trigger("change"); 
        $("#pendidikan_terakhir").val(pendidikan_terakhir).trigger("change"); 
        $("#kota").html(`<option value="${id_kota}">${kecamatan}, ${kota}, ${provinsi}</option>`).val(id_kota).trigger("change"); 
    })
